...
9. Preview at both desktop and a 430 px mobile viewport when browser tooling is available. Reject horizontal overflow, clipped text, or multi-line section labels caused by layout constraints.
10. Separate generation from outward actions. Before publishing or sending to Feishu/Lark, confirm the destination, message content, and sending identity. Preserve existing hosting architecture and dated URLs.
...
public/og.png                 optional link-preview image
```
...
- If the briefing is private-by-link, add `noindex,nofollow,noarchive` and disallow crawling in `robots.txt`; clarify that this is not access control.
