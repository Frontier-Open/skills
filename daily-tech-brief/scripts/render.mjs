...
const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(issue.brand)} · ${escapeHtml(issue.date)}</title><meta name="description" content="${escapeHtml(description)}"><meta name="robots" content="noindex,nofollow,noarchive"><link rel="canonical" href="${escapeHtml(canonical)}"><meta property="og:type" content="article"><meta property="og:site_name" content="${escapeHtml(issue.brand)}"><meta property="og:title" content="${escapeHtml(issue.headline)}"><meta property="og:description" content="${escapeHtml(description)}"><meta property="og:url" content="${escapeHtml(canonical)}"><meta property="og:image" content="${escapeHtml(ogUrl)}"><meta property="og:image:secure_url" content="${escapeHtml(ogUrl)}"><meta property="og:image:type" content="image/png"><meta property="og:image:width" content="1731"><meta property="og:image:height" content="909"><meta property="og:image:alt" content="${escapeHtml(issue.headline)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escapeHtml(issue.brand)} · ${escapeHtml(displayDate)}"><meta name="twitter:description" content="${escapeHtml(issue.headline)}"><meta name="twitter:image" content="${escapeHtml(ogUrl)}"><style>${css}.time{white-space:nowrap;text-align:right}.time a{margin-left:16px;color:var(--accent);text-decoration:none}.time a:hover{text-decoration:underline;text-underline-offset:4px}</style></head><body><div class="page" data-day="${escapeHtml(issueDay)}"><header><div class="topline"><div class="brand"><i class="dot"></i>${escapeHtml(issue.brand)}</div><div class="time">${escapeHtml(displayDate)} <a href="/">往期 ↗</a></div></div><div class="hero"><div><div class="eyebrow">Today's signal · 今日信号</div><h1>${escapeHtml(issue.headline)}</h1></div><div class="hero-side"><strong>${escapeHtml(issue.dek)}</strong><p>不是榜单搬运。只保留与读者真正相关、可追溯的信号。</p></div></div></header><main><section class="section"><div class="section-head"><div class="section-no">01 / SIGNAL</div><h2>科技与商业</h2><div class="section-note">过去 24 小时 + 一条近期深读</div></div><div class="signal-grid">${signalCards}</div></section><section class="section"><div class="section-head"><div class="section-no">02 / BUILD</div><h2>开源雷达</h2><div class="section-note">GitHub 日榜 + HelloGitHub 中文精选</div></div><div class="repo-list">${repoRows}</div></section>${issue.products.length ? `<section class="section"><div class="section-head"><div class="section-no">03 / SHIP</div><h2>Product Hunt 今日上榜</h2><div class="section-note">官方 Feed；票数未公开则不猜</div></div><div class="product-grid">${productCards}</div></section>` : ""}<aside class="action"><div class="action-label">今日思考</div><p>${escapeHtml(issue.topic)}</p></aside></main><footer><div>${escapeHtml(issue.brand)} · ${escapeHtml(issue.date)}</div><div class="right">数据截取：${escapeHtml(issue.generated_at)}。波动指标以该时间为准${warningText}</div></footer></div></body></html>`;
