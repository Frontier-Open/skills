...
test("renders a compact Feishu card with an uploaded cover", () => {
  ...
  assert.equal(card.elements.at(-1).actions[1].url, documentUrl);
  assert.equal(card.elements.at(-1).actions[0].type, "primary");
  assert.equal(card.elements.at(-1).actions[1].type, "default");
  ...
  assert.equal(card.elements.at(-1).actions[0].text.content, "查看网页版 · 10分钟");
  assert.equal(card.elements.at(-1).actions[1].text.content, "飞书文字版");
