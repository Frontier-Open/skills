...
FORMAT_RULES = {
    "bulletin": {"body_min": 900, "body_max": 1500, "sources": 3, "primary": 1, "inline": 2},
    "report": {"body_min": 1600, "body_max": 2800, "sources": 4, "primary": 2, "inline": 3},
    "profile": {"body_min": 2400, "body_max": 4200, "sources": 6, "primary": 0, "inline": 4},
}
TITLE_HYPE_BLACKLIST = (
    "刚刚",
    "突发",
    "炸裂",
    "杀疯了",
...
def validate_record(
    ...
    *,
    ...
    cover_headline = _text(headlines, "cover", "$.headlines", issues, minimum=4, maximum=24)
    ...
    title_entries = [("$.headlines.primary", primary_headline), ("$.headlines.cover", cover_headline)] + [
        (f"$.headlines.candidates[{index}]", candidate)
        for index, candidate in enumerate(candidates)
    ]
    seen_hype_paths: set[tuple[str, str]] = set()
    for title_path, title in title_entries:
        for phrase in TITLE_HYPE_BLACKLIST:
            marker = (title, phrase)
            if phrase in title and marker not in seen_hype_paths:
                _add(
                    issues,
                    "error",
                    "headline.hype_blacklist",
                    title_path,
                    f"删除固定放大词“{phrase}”，改写为可核实变化",
                )
                seen_hype_paths.add(marker)
    ...
    primary_minimum = FORMAT_RULES.get(article_format, {}).get("primary", 1)
    primary_count = source_kinds.count("primary")
    if primary_count < primary_minimum:
        _add(
            ...
            "$.sources",
            f"{article_format or '文章'} 至少需要 {primary_minimum} 个一手来源，当前为 {primary_count}",
        )
    ...
    if public_source_ids_raw is not None:
        if not isinstance(public_source_ids_raw, list):
            _add(issues, "error", "schema.type", "$.public_source_ids", "必须是 3–5 个来源 ID 的数组")
        else:
            if not 3 <= len(public_source_ids_raw) <= 5:
                _add(
                    issues,
                    "error",
                    "source.public_count",
    ...
    for index, claim_value in enumerate(claims_raw):
        ...
        if kind in {"fact", "quote"} and not linked_sources:
            _add(issues, "error", "claim.source_required", f"{path}.source_ids", "事实和引语必须关联来源")
        if kind == "forecast" and not linked_sources:
            _add(issues, "warning", "claim.forecast_unanchored", f"{path}.source_ids", "预测最好关联事实依据")
